...
export default async function ArtifactsPage({
  ...
  return (
    <PageTransition>
      <div
        dir={isAr ? "rtl" : "ltr"}
        className="min-h-screen bg-bg-primary papyrus-texture w-full flex flex-col"
      >
        {/* Header */}
        <header className="sticky top-0 z-40 bg-bg-secondary/90 backdrop-blur border-b border-gold p-4 flex items-center justify-between">
          <Link
            href={`/${locale}/sections`}
            className={`text-gold hover:text-gold-light flex items-center gap-2 text-sm ${isAr ? "font-(family-name:--font-almarai) flex-row-reverse" : "font-sans"}`}
          >
        ...
        <main className="flex-1 max-w-[1280px] w-full mx-auto px-6 py-8">
          ...
          <div className="text-center mb-10">
            <div className="w-16 h-16 mx-auto bg-bg-card rounded-full flex items-center justify-center border border-gold mb-4 shadow-[0_0_15px_rgba(201,168,76,0.15)]">
              <span className="text-2xl">{meta.icon}</span>
            </div>
            <h1
              className={`text-3xl text-text-primary mb-4 ${isAr ? "font-(family-name:--font-almarai)" : "font-(family-name:--font-inter)"}`}
            >
              {sectionName}
            </h1>
            <div className="h-px w-32 bg-gold mx-auto opacity-50" />
          </div>

          {/* Search */}
          <div className="max-w-2xl mx-auto mb-12 relative group">
            <Search
              className={`absolute ${isAr ? "right-4" : "left-4"} top-1/2 -translate-y-1/2 text-gold w-5 h-5`}
            />
            <input
              type="text"
              placeholder={dict.artifacts.searchPlaceholder}
              dir={isAr ? "rtl" : "ltr"}
              className={`w-full bg-bg-card border border-border rounded-full py-3 ${isAr ? "pr-12 pl-4 font-(family-name:--font-almarai) text-right" : "pl-12 pr-4"} text-text-primary focus:outline-none focus:border-gold transition-colors group-hover:shadow-[0_0_15px_rgba(201,168,76,0.1)]`}
            />
          </div>
