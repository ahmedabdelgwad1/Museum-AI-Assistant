...
export default async function ArtifactDetailPage({ params }: { params: Promise<{ locale: string, artifactId: string }> }) {
  ...
  return (
    <PageTransition>
      <div className="min-h-screen bg-bg-primary papyrus-texture w-full flex flex-col md:flex-row">

        {/* Left Column - Details */}
        <div className="flex-1 md:w-[60%] p-6 relative overflow-y-auto pb-24 md:pb-6">
          <div className="flex justify-between items-center mb-6">
            <Link href={`/${locale}/sections/${artifact.section_number || 38}`} className="inline-block text-gold hover:text-gold-light font-sans text-sm">
              {dict.detail.back}
            </Link>
          ...
          <div className="relative w-full h-[300px] md:h-[400px] border border-border rounded-xl overflow-hidden mb-8 shadow-[0_0_20px_rgba(201,168,76,0.1)] p-1 bg-bg-card">
            {artifact.image_url ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center opacity-20 border border-dashed border-gold rounded-lg">
                <svg viewBox="0 0 100 100" width="80" height="80" fill="vargold">
          ...
          <h1 className={`text-4xl text-gold mb-2 leading-tight ${locale === 'ar' ? 'font-(family-name:--font-almarai) text-right' : 'font-(family-name:--font-inter)'}`}>
            {locale === 'en' ? artifact.artifact_name_en : artifact.artifact_name_ar}
          </h1>
          <h2 className={`text-xl text-text-secondary mb-6 ${locale === 'ar' ? 'font-(family-name:--font-inter) text-right' : 'font-(family-name:--font-almarai) text-2xl'}`}>
          ...
          <div className="flex flex-wrap gap-3 mb-8">
            {[
              ...
              <div key={i} className="bg-bg-card border border-gold px-3 py-1 rounded-full text-xs flex items-center gap-2" dir="ltr">
                <span>{pill.icon}</span>
                <span className="text-text-primary font-sans">{pill.text}</span>
              </div>
            ))}
          </div>

          <div className="prose prose-invert max-w-none text-text-primary font-(family-name:--font-body) text-lg leading-relaxed mb-8">
            <p>{description}</p>
          </div>
        </div>

        {/* Right Column - Chat Panel */}
        <div className="w-full md:w-[40%] bg-bg-card md:h-screen sticky top-0 border-t md:border-t-0 md:border-l border-border shadow-[-10px_0_20px_rgba(0,0,0,0.2)]">
